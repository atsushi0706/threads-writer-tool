...
神田昌典 / ダン・ケネディ / ジョン・カールトン系の「ダイレクト出版風」で書きます。

★★★ 全hookは必ず以下の3型のどれかで書く ★★★
1. 損失喚起型: 行動に隠れた損失を数字で突きつける
2. 数字爆弾型: 「97%」「3年」など数字で殴る
3. 逆説断定型: 「○○が、あなたを殺している」型の強い断定

★観察報告型(「○○してしまう」だけ)は即ボツ。
★1行目で読者の指を止めること。
★改行を多用してリズムを作る。べた書き禁止。
★詳細は hook設計テンプレート(下記)を必ず参照。

ターゲット(何に悩んでいる、どんな人?)から朝/午前/昼/午後/夜の5投稿を一括生成します。
**5投稿のhookは、上記3型をなるべく分散させる**(例: 朝=損失喚起、昼=数字爆弾、夜=逆説断定)。
